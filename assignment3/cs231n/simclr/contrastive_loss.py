...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_zi = torch.linalg.norm(z_i)
    norm_zj = torch.linalg.norm(z_j)
    norm_dot_product = torch.dot(z_i, z_j) / (norm_zi * norm_zj)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        def loss(z_i, z_j, i):
            ...
            for k in range(2*N):
                if k != i:
                    denom += torch.exp(sim(z_i, out[k]) / tau)
            return -torch.log((torch.exp(sim(z_i, z_j)/tau))/denom)

        total_loss += loss(z_k, z_k_N, k) + loss(z_k_N, z_k, k+N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    N, D = out_left.shape
    pos_pairs = torch.bmm(out_left.view(N, 1, D),
                          out_right.view(N, D, 1)).view(N, 1)

    left_norm = torch.linalg.norm(out_left, dim=1).unsqueeze(1)
    right_norm = torch.linalg.norm(out_right, dim=1).unsqueeze(1)

    pos_pairs = pos_pairs / left_norm
    pos_pairs = pos_pairs / right_norm
...
def compute_sim_matrix(out):
    ...
    out_norm = torch.linalg.norm(out, dim=1).unsqueeze(1)
    sim_matrix = sim_matrix / out_norm
    sim_matrix = sim_matrix / out_norm.T
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.sum(exponential, axis=1).unsqueeze(1)
    ...
    positive_pairs = sim_positive_pairs(out[0:N], out[N:])
    ...
    numerator = torch.vstack((positive_pairs, positive_pairs))
    numerator = torch.exp(numerator/tau)
    ...
    loss = - 1 / 2 / N * torch.sum(torch.log(numerator / denom))
