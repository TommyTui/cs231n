...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = dnext_h/np.power(np.cosh(tmp), 2)
    dx = dtanh.dot(Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        grad_cache.append(cache_t)
        h[:, t, :] = prev_h

    cache = (N, T, D, H, grad_cache)
