...
def compute_saliency_maps(X, y, model):
    ...
    y_pred = model(X)
    true_score = y_pred.gather(1, y.view(-1, 1)).squeeze()
    true_score.sum().backward()
...
def make_fooling_image(X, target_y, model):
    ...
    iteration = 1
    while(y_pred.argmax(1) != target_y):
        print(f'iteration {iteration}')
        y_pred[0, target_y].backward()
        with torch.no_grad():
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    img.requires_grad_()
    y_pred = model(img)
    (y_pred[:, target_y] - l2_reg * img.pow(2).sum()).backward()
    with torch.no_grad():
